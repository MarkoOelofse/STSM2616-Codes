...
ggplot(plot_data, aes(x = theta_mom_estimates)) +
  ...
  labs(title = "Histogram of MoM Estimates for θ (Rayleigh Distribution)",x = "θ estimate", y = "Density") +
  theme_test()
...
ggplot(plot_data, aes(x = theta_mom_estimates)) +
  geom_histogram(aes(y = after_stat(density)),bins = 50, fill = "blue", colour = "black") +
  geom_vline(xintercept = theta_true, colour = "red",linetype = "dashed",size = 1) +
  labs(title = "Histogram of MoM Estimates for θ (Rayleigh Distribution)",
       x = "θ estimate",
       y = "Density") +
  theme_test()
